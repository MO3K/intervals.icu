...
# Path to Netscape-format cookie file for endpoints that require browser session auth
# (e.g. /api/activity/{id}/analyze which is not part of the public /api/v1 API)
...
@mcp.tool()
async def reprocess_activity(activity_id: str, api_key: str | None = None) -> str:
    """Re-analyze intervals for a specific activity on Intervals.icu.

    Triggers the same analysis as clicking the "Analyze" button in the UI,
    recalculating intervals based on laps.

    NOTE: This uses the internal web endpoint /api/activity/{id}/analyze (not the public
    /api/v1 API) which requires browser session cookie authentication. Set COOKIE_FILE
    in .env to the path of a Netscape-format cookie file exported from the browser.
    The public API does not yet expose this functionality — see:
    https://forum.intervals.icu/t/exposing-activity-id-analyze-in-the-api/116616

    Args:
        activity_id: The Intervals.icu activity ID (e.g. 'i117351693')
        api_key: Unused for this endpoint (kept for API consistency)
    """
    cookie_file = COOKIE_FILE
    if not cookie_file:
        return (
            "COOKIE_FILE is required for reprocess_activity. "
            "Set COOKIE_FILE in .env to the path of a Netscape-format browser cookie file."
        )

    # Parse Netscape cookie file and build cookie dict
        ...
        with open(cookie_file) as f:
            for line in f:
                ...
                if len(parts) == 7:
                    ...
                    if "intervals.icu" in domain and "forum" not in domain:
                        cookies[name] = value
    except OSError as e:
        return f"Error reading cookie file '{cookie_file}': {e}"

    if not cookies:
        return f"No intervals.icu cookies found in '{cookie_file}'."

    url = f"https://intervals.icu/api/activity/{activity_id}/analyze"
    payload = {
        "useLaps": True,
        "keepAllLaps": True,
        "keepIntervals": False,
        "keepCustomFields": True,
        "useWorkoutSteps": False,
    }
    headers = {
    ...
    try:
        ...
        return f"Activity {activity_id} intervals successfully re-analyzed."
    except httpx.HTTPStatusError as e:
        return f"Error analyzing activity {activity_id}: HTTP {e.response.status_code} - {e.response.text}"
    except httpx.RequestError as e:
        return f"Error analyzing activity {activity_id}: {e}"
