...
class Value:
    ...
    def __str__(self) -> str:
        ...
        if self.start is not None and self.end is not None:
            formatted_start = float_to_str(self.start)
            formatted_end = float_to_str(self.end)
            if self.units in [ValueUnits.PERCENT_HR, ValueUnits.PERCENT_MMP, ValueUnits.PERCENT_LTHR,
                               ValueUnits.PERCENT_PACE, ValueUnits.PERCENT_FTP]:
                val += f"{formatted_start}-{formatted_end}% "
            else:
                val += f"{formatted_start}-{formatted_end} "
        if self.value is not None:
