...
**Method:** calculate the decision-useful value only on the central, steady Z2 main block. Exclude warmup, approach, return, cooldown, pauses, and any pace/interval block **unless** a segment is demonstrated by actual pace/HR/terrain to be part of the same steady load. For route-envelope segments, do not infer terrain equivalence from lap-average pace and HR alone: inspect the raw altitude/grade stream when available. If approach or return contains a distinct hill/grade distribution absent from the central block, exclude it from the primary pace:HR decoupling and report a secondary `core + envelope` sensitivity value. For a multi-intensity workout, never calculate one decision value across easy, HM/threshold, and recovery blocks: segment it first. A controlled HM/threshold block may receive its own pace:HR drift as a session-execution metric, but it is not an aerobic readiness gate; post-intensity easy running is likewise affected by HR recovery kinetics. Split each selected homogeneous block into equal-duration halves, then calculate pace and HR time-weighted within each half. The activity-level decoupling reported by Intervals.icu remains useful context, but it is not the readiness gate because it includes the full workout.
